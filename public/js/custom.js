...
function increaseCount(a, b) {
    ...
    // This if condition checks that the number of tickets
    // is less or equal to the number of tickets allowed for per user
    // Added by RAHUL
    if (b.previousElementSibling.getAttribute("max") <= value) {
        alert("Number of tickets per user limit reached");
    } else {
        value++;
    }
    input.value = value;
    // The next line changes the total number of tickets -> Added by RAHUL
    // document.getElementById("numberOfTickets").innerText = value;
    // let amount = document.getElementById("eventTicketPrice").innerHTML;
    // document.getElementById("totalCost").innerHTML = ticketprice * value;
}
...
// Early Bird Discount Edit Switch Button -> Added by RAHUL

$(document).ready(function () {
    $("#bird-discount-edit").on("change", function (e) {
        const isOn = e.currentTarget.checked;

        if (isOn) {
            $("#bird-discount-wrapper-edit").show();
        } else {
            $("#bird-discount-wrapper-edit").hide();
        }
    });
});
...
//Single Ticket Edit Per level Switch Button -> Added by RAHUL

$(document).ready(function () {
    $("#is-restrict-total-ticket-edit").on("change", function (e) {
        const isOn = e.currentTarget.checked;

        if (isOn) {
            $("#total_tickets_display_edit").hide();
        } else {
            $("#total_tickets_display_edit").show();
        }
    });
});
...
// Single Ticket Edit Per User Switch Button -> Added by RAHUL

$(document).ready(function () {
    $("#is-restrict-ticket-per-user-edit").on("change", function (e) {
        const isOn = e.currentTarget.checked;

        if (isOn) {
            $("#total_tickets_per_user_edit").hide();
        } else {
            $("#total_tickets_per_user_edit").show();
        }
    });
});
